...
const { ensureAdmin, ensureOrgAccess, isSupplierRole, ensureSupplierOrAdmin, ensureSupplierPermission } = require('../utils/access')
const { createBuyerInfo, ensureContractDocument, isDeliverableProductItem, validateDeliveryTotals } = require('../utils/contract-document')
...
async function list({ params, auth }) {
  if (isSupplierRole(auth?.role)) {
    await ensureSupplierPermission(auth, ['view_contract_detail'], '当前账号没有合同查看权限')
  }
...
async function detail({ params, auth }) {
  ...
  return {
    ...
    supplier_bank_account_name: result.data.supplier_bank_account_name || result.data.supplier_name || supplier?.name || '',
...
async function update({ params, auth }) {
  ...
  if (updates.product_items || updates.delivery_rows || updates.appendix_images) {
    const normalizedCandidate = ensureContractDocument({ ...existing.data, ...updates }, await loadSupplier(existing.data.supplier_org_id))
    validateDeliveryTotals(normalizedCandidate)
    updates.product_items = normalizedCandidate.product_items
    updates.delivery_rows = normalizedCandidate.delivery_rows
    updates.appendix_images = normalizedCandidate.appendix_images
  }
...
async function exportWord({ params, auth }) {
  ...
  const contract = {
    ...
    supplier_bank_account_name: contractResult.data.supplier_bank_account_name || contractResult.data.supplier_name || supplier?.name || '',
  ...
  validateDeliveryTotals(contract)
...
async function pushConfirm({ params, auth }) {
  ...
  validateDeliveryTotals(ensureContractDocument(contract, await loadSupplier(contract.supplier_org_id)))
...
async function uploadSigned({ params, auth }) {
  ...
  const hadExistingBatches = batchCount.total > 0

  const org = await loadSupplier(contract.supplier_org_id)
  const normalizedContract = ensureContractDocument(contract, org)
  validateDeliveryTotals(normalizedContract)
  const autoGeneratedBatches = hadExistingBatches
    ? []
    : buildAutoGeneratedBatches(contract, normalizedContract, new Date().toISOString())

  for (const batchDoc of autoGeneratedBatches) {
    await getCollection(BATCHES_COLLECTION).add({ data: batchDoc })
  }

  const hasBatches = hadExistingBatches || autoGeneratedBatches.length > 0
  const snapshot = org
    ? {
...
async function confirm({ params, auth }) {
  ...
  if (isSupplierRole(auth?.role)) {
    await ensureSupplierPermission(auth, ['view_contract_detail'], '当前账号没有合同确认权限')
  }
...
async function create({ params, auth }) {
  ...
  const doc = {
    ...
    supplier_bank_account_name: params.supplier_bank_account_name || params.payment_account_name || params.supplier_name || '',
    ...
    appendix_images: Array.isArray(params.appendix_images) ? params.appendix_images : [],
    created_at: now,
    updated_at: now
  }
...
function buildAutoGeneratedBatches(contract, normalizedContract, now) {
  ...
  const productItems = Array.isArray(normalizedContract.product_items)
    ? normalizedContract.product_items.filter(isDeliverableProductItem)
    : []
  const invalidRows = []
  const batchDocs = []
  ...
  if (invalidRows.length > 0) {
    const error = new Error(`交付计划第 ${invalidRows.join('、')} 行缺少有效日期，请先补全后再上传已签合同`)
    error.code = 400
    throw error
  }
