...
const { ensureAdmin, ensureAdminPermission, ensureOrgAccess } = require('../utils/access')
...
async function create({ params, auth }) {
  ensureAdmin(auth)
  await ensureAdminPermission(auth, 'module_contract')
  ensureBatchManager(auth)
...
async function update({ params, auth }) {
  ...
  const now = new Date().toISOString()
  const changeReason = String(updates.change_reason || '').trim()
  const changeDetails = []
  ...
  if (updates.planned_date !== undefined && updates.planned_date !== previous.planned_date) {
    changeDetails.push(`交付日期：${previous.planned_date || '-'} -> ${updates.planned_date || '-'}`)
  }

  if (updates.note !== undefined && String(updates.note || '') !== String(previous.note || '')) {
    changeDetails.push('批次备注已更新')
  }

  if (Array.isArray(allowedFields.parts)) {
    const currentParts = previous.parts || []
    allowedFields.parts.forEach((part) => {
      const currentPart = currentParts.find((item) => item.part_id === part.part_id)
      if (!currentPart) {
        return
      }
      if (Number(part.planned_qty || 0) !== Number(currentPart.planned_qty || 0)) {
        changeDetails.push(`${part.part_name || '未命名配件'}：${Number(currentPart.planned_qty || 0)} -> ${Number(part.planned_qty || 0)}`)
      }
    })
  }
  ...
  if (!changeReason) {
    const error = new Error('请填写本次交付计划调整原因')
    error.code = 400
    throw error
  }

  allowedFields.change_logs = [
    {
      change_id: `chg_${Date.now()}`,
      reason: changeReason,
      details: changeDetails,
      operator_id: auth.user_id || '',
      operator_name: auth.name || auth.role_name || '甲方管理员',
      created_at: now
    },
    ...((previous.change_logs || []).slice(0, 49))
  ]
...
function ensureBatchManager(auth) {
  if (!auth || !['super_admin', 'admin'].includes(auth.role)) {
    const error = new Error('只有甲方管理员可以调整交付计划')
    error.code = 403
    throw error
  }
}
